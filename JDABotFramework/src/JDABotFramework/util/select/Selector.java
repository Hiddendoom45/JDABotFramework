package JDABotFramework.util.select;
...
public class Selector {
	...
	public static boolean parseSelection(MessageReceivedEvent event){
		...
		if(selections.containsKey(key(event))){
			...
			if(event.getMessage().getContent().equals("exit")){
				...
				JDAMessage.sendTempMessage(event, event.getAuthor().getName()+" exited menu",20);//send message to update status
				return true;
			}
			//if typical selection event
				...
				//if not send message that they selected the wrong one
					...
					if(select.tries>=3){//to avoid infinate loops, fails after 3 attempts
						...
						JDAMessage.sendTempMessage(event, JDAMessage.FormatMessage(event, "%userMention% Exited selection menu due to 3 incorrect responses"),20);
					}
					else{
						select.tries++;
						if(select.file==null){
							select.messageID=JDAMessage.sendMessage(event, select.errMessage).getId();
						}
						else{
							select.messageID=JDAMessage.sendFile(event, select.errMessage, select.file).getId();
						}
					}
				}
				return true;
	...
	public static void setSelection(Select selection,MessageReceivedEvent event){
		if(selection.file==null){
			selection.messageID=JDAMessage.sendMessage(event,selection.message).getId();
		}
		else{
			selection.messageID=JDAMessage.sendFile(event, selection.message, selection.file).getId();
		}
