...
public class ArgumentParser {
	...
	protected ArgumentParser(BotGlobalConfig config){
		this.config=config;
	}
	public ArgContainer handleArguments(String raw){
		...
		for(String s:array){
			if(s.startsWith(config.getOverrideArg())){
				if(!arg.equals("")){
			...
			else if(s.startsWith(config.getOverridePrefix())){
				command=s.substring(config.getOverridePrefix().length());
			}
			else{
				values+=s+",";
			}
